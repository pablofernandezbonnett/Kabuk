...
All primary keys use `BIGINT` values generated by PostgreSQL as identity columns. The API may expose different public IDs, so its contract does not depend on this database detail.
...
Each hotel has its own room types. Two hotels may both offer a room called "Deluxe Room", but they are different room type records because their details, price, and availability can differ.
